...
// 读取发布签名配置（app/android/key.properties，已 gitignore）
// 不存在时回退 debug 签名，保证未配置签名时仍可构建
...
android {
    ...
    defaultConfig {
        ...
        applicationId = "xyz.qiaosheng.app"
    ...
    buildTypes {
        release {
            // 正式发布签名：存在 key.properties 时使用专属签名，否则回退 debug 签名以保证构建可用
